...
//                'is_fh' => 0,//已分红的订单
    ...
    public function cYj()
    {
        ...
        foreach ($memberInfo as $info) {
            ...
            // 更新佣金钱包余额
            $rest = M('Member')->where(['id' => $info['id']])->save(['commission' => ['exp', 'commission+' . $money*($this->systemInfo['crate']/100)]]);
        }
    }
    ...
    private function sum($id){
        ...
        $group = [];
        $dataArr =  $this->difference($id,1,$group);
    ...
    private function difference($id,$level = 1,&$group){
        ...
        if(!empty($child)){
            ...
            foreach($child as $k => $v){
                $this->difference($v['id'],$level+1,$group);
            }
        }
        return $group;
    }
